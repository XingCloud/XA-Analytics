...
class Analytics.Views.Ads.IndexView extends Backbone.View
  template: JST['backbone/templates/ads/ad_add_index']
  events:
    "click #add-ad" : "add_ad"
  ...
  add_ad: () ->
    ...
    if @check_field()
      ...
      model.save({}, {wait:true, success: (model ,resp) ->
        ...
        _this.xa_log(date, channel, Math.ceil(parseFloat(fee)*100))
      })
  ...
  update_fee: (ev) ->
    ...
    if @check_fee(fee, $(ev.currentTarget).parent().find("span.error-message"))
      model.save({fee:fee},{wait:true, success:(model, resp)->
        _this.xa_log(model.get("date"), model.get("channel"), Math.ceil(parseFloat(model.get("fee"))*100))
        redraw()
      })
