class ProjectUsersController < ProjectBaseController
  ...
  def index
    ...
    if true or @login_user.role == "admin" or (not project_login_user.nil? and project_login_user.role != "mgriant")
  ...
  def update
    ...
    if true or @login_user.role == "admin" or (not project_login_user.nil? and project_login_user.role == "admin")
...
private
  def find_login_user
    @login_user = User.find_by_name(session[:cas_user])
